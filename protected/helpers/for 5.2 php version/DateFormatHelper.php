...
class DateFormatHelper {
    ...
    public static function getDateFromCommonTextView($dateCommonTextView) {
        $date = FixedDateTime::createFromFormat(self::COMMON_DATE_FORMAT, $dateCommonTextView);
        $date->setTime(0, 0, 0);
        return $date;
    }
    
    public static function getDateFromDBView($dateDBView) {
        $date = FixedDateTime::createFromFormat(self::DB_DATE_FORMAT, $dateDBView);
        $date->setTime(0, 0, 0);
        return $date;
    }
    ...
    public static function getCopyOfDate($date) {
        $f = 'd-m-Y H:i:s';
        $copy = FixedDateTime::createFromFormat($f, $date->format($f) );
        return $copy;
    }
